setwd('/home/wanyiyang/ENIGMA_STUDY_DATA/7association/arg_asso/')
library(ggplot2)
library(pheatmap)
library(dplyr)
library(readr)
...
plotAssociationsDag3HeatmapV2 <- function(inData, phenosToPlot, statToPlot, featuresToPlot, 
  ...
  # 筛选需要绘制的表型和特征
  inData <- inData %>%
    filter(phenotype %in% phenosToPlot & arg_gene %in% featuresToPlot)
  
  # 创建矩阵：行是特征，列是表型
  heatmap_data <- inData %>%
    select(phenotype, arg_gene, !!sym(statToPlot)) %>%
    tidyr::pivot_wider(
...
generate_heatmap <- function(data_file, output_file, title, stat_to_plot = "estimate") {
  ...
  # 确保数据格式符合要求
  if (!"metadata_var" %in% colnames(data) || !"arg_gene" %in% colnames(data)) {
    stop("数据文件缺少必要的列：metadata_var 或 arg_gene")
  }
  ...
  features_to_plot <- unique(data$arg_gene)
